...
export const formatDateToRef = (date: string | Date): string => {
  ...
  const min = String(d.getMinutes()).padStart(2, '0'); // Minute (00-59)
  const ss = String(d.getSeconds()).padStart(2, '0'); // Second (00-59)
  const ms = String(d.getMilliseconds()).padStart(3, '0'); // Millisecond (000-999)

  return `${yy}${mm}${dd}-${hh}${min}${ss}${ms}`; // e.g., "250405-123045678"
};

export const createRefNumberFromDate = (date: string | Date): string => {
  const datePart = formatDateToRef(date); // e.g., "250405-123045678"
  return `REF-${datePart}`;
};
...
export const createDocumentTransaction = async (
  ...
  const ref = createRefNumberFromDate(dateNow);
  try {
